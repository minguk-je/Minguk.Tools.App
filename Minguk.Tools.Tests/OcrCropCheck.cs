...
/// <c>--ocr-crop --image=경로.png --region=x,y,w,h [--lang=ko] [--scale=3] [--ink]</c>
/// </summary>
/// <remarks>
...
internal static class OcrCropCheck
{
    public static int Run(string[] args)
    {
        ...
        BitmapSource scaled = ink
            ? HudInk.Prepare(crop)
            : new TransformedBitmap(crop, new ScaleTransform(scale, scale));
        ...
        Console.WriteLine($"오려낸 자리 {box.X},{box.Y} {box.Width}x{box.Height} · " +
                          $"{(ink ? $"흰 글자만 남기고 {HudInk.Scale}배" : $"{scale}배로 키움")} · {ocr.Name} ({ocr.Language})");
        Console.WriteLine();
