...
/// <summary>
/// 프레임마다 새로 찾은 것을 직전 것과 이어 붙여, 잠깐 나온 헛것은 버리고 잠깐 사라진 검출은 이어 준다.
/// </summary>
/// <remarks>
/// <b>왜 필요한가</b> - 검출기는 매 프레임을 따로 본다. 그래서 한 프레임에만 30%짜리 헛것이
/// 튀고, 검출이 반 프레임 가려지면 사각형이 깜빡인다. 실측(33장 60바퀴)으로 진짜 봇은 80%대가
/// 매 프레임 같은 자리에 나오고, 헛것은 30~50%대가 한두 프레임 나왔다 사라졌다. 이 차이를
/// 쓰는 것이 추적이다. "검출이 있다" 는 판단을 프레임 하나가 아니라 짧은 시간으로 내린다.
///
...
/// 게임 검출이 그 사이 화면에서 사라지는 일은 드물다. 더 길게 잡으면 사라진 검출의 사각형이
/// 남아 그 자리를 누르게 된다.
///
...
public sealed class DetectionTracker
{
    ...
    /// <summary>지난 것과 이만큼 겹치면 같은 검출이다. 검출 짝짓기(0.5)보다 느슨하다 - 검출이 움직인다.</summary>
    public double MatchIou { get; init; } = 0.3;

    /// <summary>
    /// 안 겹쳐도 가운데가 사각형 크기의 이만큼 안에 있으면 같은 검출이다.
    /// </summary>
    /// <remarks>
    /// 640 모델은 한 번에 0.65초라 시선을 돌리면 그 사이 검출이 제 폭보다 더 옮겨 가 겹침이
    /// 0 이 된다. 겹침만 보면 새 검출로 갈라져, 옛 자리 사각형이 두 프레임 더 남아 유령이
    ...
    public IReadOnlyList<Detection> Update(IReadOnlyList<Detection> found)
    {
        ...
        // 잘 맞는 짝부터 맺는다. 검출 둘이 붙어 있을 때 엉뚱한 쪽으로 이어지는 것을 줄인다.
