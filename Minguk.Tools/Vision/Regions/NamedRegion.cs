...
/// 꼽는 셋이 그레이스케일 변환·이진화(밝기 기준)·확대(작은 글자는 키워야 읽는다)라 그 셋으로 골랐다 - 옛 것과 달리 다 기본은 꺼짐(<c>Threshold</c> 0·
...
public sealed class NamedRegion : INotifyPropertyChanged
{
    ...
    /// <summary>
    /// 이진화 밝기 기준(0~255). 0 이면 꺼짐(자른 그림 그대로 읽는다). 켜면 그레이스케일로 바꾼 뒤 이 값을 기준으로 검거나 희게 가른다.
    /// </summary>
    /// <remarks>
    /// 사용자(2026-09-18) "탄약 숫자도 잘 못 읽잖아" - PP-OCRv5 가 손질 없이도 대개 읽지만, 대비가 낮거나 배경이 겹친 HUD 는 여전히 헷갈린다.
    /// 기준을 넘으면(밝으면) 흰색, 아니면(어두우면) 검은색으로 - 글자 윤곽이 뚜렷해진다. 맞는 값은 자리마다 달라(밝은 글자·어두운 글자) 사람이 눈으로 맞춘다.
    /// </remarks>
