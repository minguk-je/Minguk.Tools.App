...
public static class ScriptApiCatalog
{
    ...
    [
        ...
        new("ReadText", "읽기", "x, y, width, height", "그 자리(0~1 비율)의 글자를 읽는다. 읽기(\"탄약\") 으로 화면에서 만들어 둔 자리(칸들을 차례로 이어), 읽기(\"탄약.현재\") 로 칸 하나를 부를 수도 있다.", ScriptApiMode.Live),
        new("ReadNumber", "숫자읽기", "x, y, width, height", "그 자리의 글자에서 숫자만 뽑는다. 숫자읽기(\"탄약.최대\") 처럼 만들어 둔 자리·칸을 부를 수도 있다. 없으면 null.", ScriptApiMode.Live),
        ...
        new("ReadNumbersAt", "숫자들읽기", "name", "이름 붙인 자리에서 읽은 숫자를 칸 순서대로 준다. 「현재」·「최대」 칸이면 [30, 40] - 믿을 범위는 스크립트가 고른다. \"자리.칸\" 이면 그 칸만.", ScriptApiMode.Live),
        new("HasTextAt", "글자있나", "name", "이름 붙인 자리(또는 \"자리.칸\")에 읽을 글자가 있는가. 스크립트 화면의 영역 패널에서 만든 이름.", ScriptApiMode.Live),
