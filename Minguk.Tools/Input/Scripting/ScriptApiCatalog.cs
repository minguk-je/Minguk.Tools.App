...
public static class ScriptApiCatalog
{
    ...
    [
        ...
        new("RunInBackground", "따로돌리기", "name, ms, work", "그 일을 다른 스레드에서 간격마다 되풀이한다 - 스크립트가 끝나거나 멈출 때까지. 줍기·물약·물러나기처럼 본 흐름과 상관없이 늘 봐야 하는 것. 안에서 난 오류는 출력에 알리고 이어 돌고, 안전장치에 걸리면 스크립트 전체가 선다.", ScriptApiMode.Live),
        ...
        new("FindTexts", "글자찾기들", "text, [name]", "그 글이 든 곳을 모두 찾는다(위에서 아래 순). 몹 이름처럼 위쪽 대상 칸과 몹 머리 위 두 곳에 뜨는 글에서 고를 때. 화면 전체는 0.3~0.6초.", ScriptApiMode.Live),
        new("PressText", "글자누르기", "text, [name], [button]", "글자찾기로 찾아 그 가운데를 누른다. 찾았으면 true. 게임 메뉴·버튼 - 글자누르기(\"사격장\").", ScriptApiMode.Live),
        new("ReadLines", "글줄들", "[name]", "그 자리(없으면 화면 전체)를 한 번 읽어 모든 줄을 자리(x·y 화면 픽셀, 너비·높이, 글)와 함께 준다. 여러 글을 한꺼번에 견줄 때 - 글자찾기들을 여러 번 부르면 그때마다 다시 읽는다.", ScriptApiMode.Live),
        ...
        new("BarFillAt", "채움", "name", "그 이름 자리(칸)의 막대 - 대상 체력바·마나·게이지 - 가 몇 할 찼는지(0~1). 왼쪽부터 색이 있는 칸이 이어진 길이로 잰다(빈 칸은 회색·검정). 대상이 없어 막대가 안 보이면 0 가까이.", ScriptApiMode.Live),
        new("BrightnessAt", "밝기", "name", "그 이름 자리(칸)의 평균 밝기(0~255). 켜진 단추와 꺼진(어두운) 단추를 가른다 - 본보기 그림은 밝기에 안 흔들려 둘을 못 가른다.", ScriptApiMode.Live),
