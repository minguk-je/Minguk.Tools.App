...
/// <summary>
/// 스크립트가 보는 검출 하나. 자리는 <b>화면 픽셀</b>이라 <c>이동(검출.중심x, 검출.중심y)</c> 에 그대로 넣는다.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed record ScriptDetection(string Name, double Score, int CenterX, int CenterY, int Width, int Height, string Caption)
{
    ...
    /// <c>조준(검출.머리x, 검출.머리y)</c> 로 쓴다.
    /// </summary>
    /// <remarks>
    /// <b>왜 가운데가 아닌가</b> - 사각형 가운데는 사람으로 치면 배다. 머리를 노리는 편이 한 발의 값이 크고,
    /// 검출이 좌우로 움직여도 머리는 덜 흔들린다(팔다리가 사각형을 넓혔다 좁혔다 하는 것은 아래쪽이다).
    ///
    ...
    /// <summary>
    /// 머리 위 글자 - <b>부를 때</b> 지금 화면에서 사각형 위를 잘라 읽는다(검출마다 한 번만 읽고 기억). 이름표가 없는 게임·못 읽으면 빈 글.
    /// </summary>
    /// <remarks>
    /// 모든 게임이 이름표를 띄우지 않아 늘 읽지 않는다(사용자, 2026-09-17) - 한때 「이름표 읽기」 체크로 검출마다 읽었다.
    /// 검출을 찾은 화면보다 조금 뒤(최대 0.25초) 화면을 읽는다 - 이름표 칸은 사각형보다 넉넉하다(<see cref="Minguk.Tools.Vision.Ocr.NameplateRegion"/>).
    /// </remarks>
    ...
    /// <summary>이 검출을 준 API 의 이름표 읽기. 같은 실행의 검출은 한 벌을 나눠 가져 같음 비교에 안 걸린다.</summary>
