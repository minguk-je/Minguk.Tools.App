...
internal sealed class PaddleDetector : IDisposable
{
    ...
    /// <summary>정규화(0~1 단위, BGR) - 모델 벌마다 다르다(v5 ImageNet · v6 0.5/0.5).</summary>
    ...
    public PaddleDetector(string modelPath, bool useGpu, float[]? mean = null, float[]? std = null)
    {
        _engine = new OnnxDmlEngine(modelPath, useGpu: useGpu);
        _mean = mean ?? [0.485f, 0.456f, 0.406f];
        _std = std ?? [0.229f, 0.224f, 0.225f];
    }
    ...
    public IReadOnlyList<DbBox> Detect(BgraImage image)
    {
        ...
        for (var y = 0; y < height; y++)
        {
            for (var x = 0; x < width; x++)
            {
                ...
                for (var c = 0; c < 3; c++)
                    _tensor[(c * plane) + target] = ((resized.Pixels[source + c] / 255f) - _mean[c]) / _std[c];
            }
        }
