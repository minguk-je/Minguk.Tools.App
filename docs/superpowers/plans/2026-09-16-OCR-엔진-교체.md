...
| `docs/몹-검출.md` · `CLAUDE.md` · 스펙 | 문서 | 7 |
...
(a) 도구 줄의 `<dxb:BarEditItem Content="언어" …>…</dxb:BarEditItem>` 를 통째로 지운다. `이름표 읽기` 의 `Hint` 를 `"찾은 몹마다 머리 위 이름표를 읽는다."` 로.
...
**Files:**
- Modify: `docs/몹-검출.md` (## 글자 읽기 (OCR) 절)
- Modify: `CLAUDE.md` (어댑터 표, 검증 명령)
- Modify: `docs/superpowers/specs/2026-09-16-OCR-엔진-교체-design.md`

- [ ] **Step 1: `docs/몹-검출.md`** - "## 글자 읽기 (OCR)" 절의 첫 문단부터 "HUD 숫자" 항목까지를 아래로 바꾸고(`<…>` 는 Task 5 숫자로 채운다), "이름 붙인 자리" 항목의 그리드 열 목록에서 `전처리` 를 빼고, 끝의 `도구:` 줄을 고친다.
