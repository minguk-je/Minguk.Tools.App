...
### 추론 속도 - 실측 (GTX 1060 3GB)

`dotnet run --project Minguk.Tools.Tests -- --detect-bench` 로 잰다. libtorch 와 학습한
모델이 있어야 돌아서 평소 검증에는 안 낀다.

| 넣은 크기 | 한 장 | fps | 찾은 것(2개 중) |
|---|---|---|---|
| 160x90 | 121 ms | 8.3 | 1 |
| **320x180** | **220 ms** | **4.5** | **2** |
| 480x270 | 439 ms | 2.3 | 2 |
| 640x360 | 587 ms | 1.7 | 2 |
| 960x540 | 1,064 ms | 0.9 | 2 |
| 1920x1080 | 4,724 ms | 0.2 | 1 |

곁들여: libtorch 올리기 0.65초 · 모델 읽기 2.9초(68MB). 둘 다 한 번만 든다.

**AutoFormerV2 는 넣은 그림을 줄이지 않고 그대로 본다.** 값이 픽셀 수에 거의 비례한다
(픽셀 32배 → 22배). 트랜스포머라 그렇다. 그래서 **넣기 전에 줄이는 것이 유일한 손잡이**다.

- **파일을 읽는 값은 0%** 다(1080p 에서 4.3ms / 4,724ms). 프레임을 파일로 안 떨어뜨리고
  픽셀을 바로 넘기게 고쳐도 얻는 것이 없다 - 지금 API 가 경로만 받는 것은 문제가 아니다.
- GPU 는 쓰고 있다. 추론 중에 `nvidia-smi` 로 봤을 때 1,424 MiB 를 물고 있었다.
  느린 것은 CPU 로 돌아서가 아니다.
- **학습한 크기 근처에서 넣어야 잘 찾는다.** 320x200 으로 학습한 모델이 160x90 과
  1920x1080 에서는 2개 중 1개만 찾았고, 320x180~960x540 에서는 둘 다 찾았다.
  크기를 바꾸려면 학습부터 그 크기로 다시 해야 한다.

**캡처 프레임마다 돌리는 실시간 겹쳐 그리기는 안 된다.** 30fps 면 한 장에 33ms 인데
가장 빠른 자리가 121ms 다. 대신 **띄엄띄엄 돌리는 것**은 된다 - 320x180 으로 줄여
0.25초에 한 번이면 몹을 찾아 클릭하는 데는 넉넉하다. 붙일 때는 그 갈래로 짠다.
...
- `--views` 와 `--detect-bench` 외에는 **커서와 키보드를 가져간다.** 돌리는 동안 손을 떼야 한다.
