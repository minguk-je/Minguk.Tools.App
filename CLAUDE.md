...
`torch.set_num_threads(코어/4, 2~8)` 로 제한한다. GPU 학습 속도는 거의 안 변한다.

### 학습 그림 캐시 - 매 바퀴 1080p 를 읽지 않는다

학습기는 매 바퀴 그림을 파일에서 읽어 줄인다. 1080p 97장 40바퀴면 3,880번이고 전부 CPU 몫이라
GPU 는 13% 만 돌고 2시간이 걸렸다. `TrainingImageCache` 가 학습 전에 라벨 있는 그림을 모델
크기로 한 번 줄여 `Datasets\몹\cache\640x360\` 에 두고 학습기에는 그 경로를 넘긴다.
라벨은 0~1 이라 그대로 맞고, 줄이는 방법은 실시간 경로(WPF)와 같다. 원본 시각이 캐시보다
뒤면 그 장만 다시 만들고, 못 만든 장은 원본을 그대로 쓴다(느릴 뿐 틀리진 않는다).
"지금 보는 그림" 표시는 원본 경로로 알려야 목록과 맞는다(`originals`). `--vision` 이 시험한다.
