...
```
캡처 모니터  ──담기──▶  데이터셋  ──찍기──▶  라벨  ──학습──▶  .onnx  ──▶  OnnxDmlEngine
 (WgcCaptureSession)      images/           labels/                        (이미 있다)
```

지금까지 된 것은 **담기와 찍기**다. 학습과 추론을 붙이는 자리는 아직 비어 있다 -
`Inference/OnnxDmlEngine` · `FramePreprocessor` 는 만들어져 있지만 부르는 곳이 없다.
