...
| `docs/퀘스트-스크립트-흐름.md` | 아이온2 퀘스트 스크립트 - 화면이 판단까지 오는 길(캡처→허브 0.1초→읽기 캐시), 영역·엔진 표, 전략 넷, 멈춤 조건, 로그 보는 법 |
...
- **미니맵 방향**(2026-09-23, 사용자 "미니맵으로 방향 보면서 처리가 가능해?"): 자동 이동이 안 되는 곳에서 미니맵 화살표로 몸 방위를, 노란 마커로 목표 방위를 읽어 걷는다
  ...
  미니맵 점은 **이름으로**(`마커방위("몹")`·`마커로가기("몹", 밀리초)`, 2026-09-25) - 색은 게임마다 `minimap.json` 의 `markers` 에, 코드에 게임 것을 박지 않는다.
...
- **검은 프레임이 5초 이어지면 창 캡처를 스스로 다시 잡는다**(`WgcCaptureSession.WatchBlackFrame`, 2026-09-26): 아이온2 가 지역 이동 로딩에서 스왑체인을 다시 만들면 그 뒤로
  검은 프레임만 온다(미리보기도 검고, 중지→시작이면 돌아온다 - 사용자 확인). 프레임은 계속 오니 「한 장도 안 올 때」 폴백으로는 못 잡아 리드백 픽셀을 24px 간격으로 훑는다.
  다시 잡기는 15초에 한 번, 프레임 번호는 잇는다. 리드백이 꺼진 세션(화면캡처만)은 안 본다.
